...
const debug = require('debug')(`lib/iptables`);
...
const createProxy = (ip, loopback, port) => {
  debug(`createProxy(): ${ip}, ${loopback}, ${port}`);
  return delAll(ip)
    .then(() => {
...
const createPassthrough = (ip) => {
  debug(`createPassthrough(): ${ip}`);
  return delAll(ip)
    .then(() => {
